...
{
    internal class GtkStorageProvider : BclStorageProvider
    {
        private Task<bool>? _initialized;
        private readonly X11Window _window;

        public GtkStorageProvider(X11Window window)
        {
            _window = window;
        }
        ...
        public override async Task<IStorageFolder?> OpenFolderPickerAsync(FolderPickerOpenOptions options)
        {
            ...
            return await await RunOnGlibThread(async () =>
            {
                var res = await ShowDialog(options.Title, _window, GtkFileChooserAction.SelectFolder,
                    false, options.SuggestedStartLocation, null,
                    null, null, false).ConfigureAwait(false);
                return res?.FirstOrDefault() is { } folder
        ...
        public override async Task<IStorageFile?> SaveFilePickerAsync(FilePickerSaveOptions options)
        {
            ...
            return await await RunOnGlibThread(async () =>
            {
                var res = await ShowDialog(options.Title, _window, GtkFileChooserAction.Save,
                    false, options.SuggestedStartLocation, options.SuggestedFileName, options.FileTypeChoices, null, options.ShowOverwritePrompt ?? false)
                    .ConfigureAwait(false);
            ...
            IEnumerable<FilePickerFileType>? filters, string? defaultExtension, bool overwritePrompt)
        {
                ...
                ConnectSignal<signal_dialog_response>(dlg, "response", (_, resp, __) =>
                {
                    ...
                    if (resp == GtkResponseType.Accept)
                    {
                        ...
                        if (action == GtkFileChooserAction.Save)
                        {
                            ...
                            for (var c = 0; c < result.Length; c++) { result[c] = NameWithExtension(result[c], defaultExtension, selectedFilter); } }
                    }
            ...
            if (initialFolder?.TryGetFullPath(out folderPath) == true)
            {
                using var dir = new Utf8Buffer(folderPath);
            ...
            if (initialFileName != null)
            {
                ...
                using var fn = action == GtkFileChooserAction.Open
                    ? new Utf8Buffer(Path.Combine(folderPath ?? "", initialFileName))
        ...
        private static string NameWithExtension(string path, string defaultExtension, FilePickerFileType? filter)
        {

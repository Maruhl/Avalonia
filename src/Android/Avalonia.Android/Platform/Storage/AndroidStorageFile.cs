...
{
    internal class AndroidStorageFile : IStorageBookmarkFile
    {
        ...
        public AndroidStorageFile(Context context, AndroidUri uri)
        {
            _context = context;
            _uri = uri;
        }

        public string Name => GetColumnValue(_context, _uri, MediaStore.Files.FileColumns.DisplayName)
        ...
        public Task<Stream> OpenRead() => Task.FromResult(OpenContentStream(_context, _uri, false)
        ...
        public Task<StorageItemProperties> GetBasicPropertiesAsync()
        {
            ...
            var projection = new[]
            {
                ...
                MediaStore.Files.FileColumns.DateModified
            };
            using var cursor = _context.ContentResolver!.Query(_uri, projection, null, null, null);

            if (cursor?.MoveToFirst() == true)
            {
                try
                {
                    var columnIndex = cursor.GetColumnIndex(MediaStore.Files.FileColumns.Size);
                    if (columnIndex != -1)
                    {
                ...
                try
                {
                    var columnIndex = cursor.GetColumnIndex(MediaStore.Files.FileColumns.DateAdded);
                    if (columnIndex != -1)
                    {
                ...
                try
                {
                    var columnIndex = cursor.GetColumnIndex(MediaStore.Files.FileColumns.DateModified);
                    if (columnIndex != -1)
                    {
